...
use App\PeriksaSatuAMPUnitTenagaPenggerak as PMAMP1UBD;
use Illuminate\Http\Request;
use App\Lib\AHelper;
use Carbon\Carbon;

class PeriksaSatuAMPUnitTenagaPenggerakCtrl extends Controller {
	public function __construct($value=''){
		$this->_s = new AHelper();
		$this->middleware('auth');
	}

	public function periksaSatuAMPUnitTenagaPenggerak(){
		$id_periksa = session('id_periksa');
		$pm_satu_amp_tenagapenggerak = PMAMP1UBD::orderBy('tgl_periksa','DESC')->where('id_periksa',$id_periksa)->first();
	...
	public function periksaSatuAMPUnitTenagaPenggerakPost(Request $request){
		...
		$q = ($request->no_id == null) ? new \App\PeriksaSatuAMPUnitTenagaPenggerak() : \App\PeriksaSatuAMPUnitTenagaPenggerak::find($request->no_id);
		$pm = $q;

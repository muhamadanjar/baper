...
use App\PeriksaSatuAMPUnitPemasokAspal as PMAMP1UBD;
use Illuminate\Http\Request;
use App\Lib\AHelper;
use Carbon\Carbon;

class PeriksaSatuAMPUnitPemasokAspalCtrl extends Controller {
	public function __construct($value=''){
		$this->_s = new AHelper();
		$this->middleware('auth');
	}

	public function periksaSatuAMPUnitPemasokAspal(){
		$id_periksa = session('id_periksa');
		$pm_satu_amp_pemasokaspal = PMAMP1UBD::orderBy('tgl_periksa','DESC')->where('id_periksa',$id_periksa)->first();
	...
	public function periksaSatuAMPUnitPemasokAspalPost(Request $request){
		...
		$ketel_tangki_aspal_foto = $request->file('ketel_tangki_aspal_foto');
		$ketel_tangki_minyak_foto = $request->file('ketel_tangki_minyak_foto');
		...
		$q = ($request->no_id == null) ? new \App\PeriksaSatuAMPUnitPemasokAspal() : \App\PeriksaSatuAMPUnitPemasokAspal::find($request->no_id);
		$pm = $q;
		...
		if ( !is_null($ketel_tangki_aspal_foto) )  {
			$fileName_ketel_tangki_aspal_foto = str_random(20) . '.' . $ketel_tangki_aspal_foto->getClientOriginalExtension();	
			$pm->ketel_tangki_aspal_foto = $fileName_ketel_tangki_aspal_foto;
			$this->_s->UploadFile($ketel_tangki_aspal_foto,$fileName_ketel_tangki_aspal_foto);
		}

		$pm->ketel_tangki_minyak_check = $request->ketel_tangki_minyak_check;
		$pm->ketel_tangki_minyak_ket = $request->ketel_tangki_minyak_ket;

		if ( !is_null($ketel_tangki_minyak_foto) )  {
			$fileName_ketel_tangki_minyak_foto = str_random(20) . '.' . $ketel_tangki_minyak_foto->getClientOriginalExtension();	
			$pm->ketel_tangki_minyak_foto = $fileName_ketel_tangki_minyak_foto;
			$this->_s->UploadFile($ketel_tangki_minyak_foto,$fileName_ketel_tangki_minyak_foto);
		}
